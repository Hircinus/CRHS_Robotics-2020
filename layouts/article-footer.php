	</div>
	<footer>
		<div id="footrow" class="row justify-content-center">
			<div class="col-lg-3 col-md-6 col-sm-12 col-12">
				<h6>Even more about us</h6>
				<hr class="foot-div">
				<a class="footlink" href="about">About</a><br>
				<a class="footlink" href="message_from_me">Message from the webmaster</a>
			</div>
			<div class="col-lg-3 col-md-6 col-sm-12 col-12">
				<h6>Useful resources</h6>
				<hr class="foot-div">
				<a class="footlink" target="_blank" href="privacy_policy">Privacy Policy <i class="fas fa-external-link-alt"></i></a><br>
				<a class="footlink" target="_blank" href="how_to_make_a_site">I wanna make a site! <i class="fas fa-external-link-alt"></i></a>
			</div>
			<div class="col-lg-3 col-md-6 col-sm-12 col-12">
				<h6>Other cool stuff</h6>
				<hr class="foot-div">
				<a class="footlink" target="_blank" href="http://www.crhs.rsb.qc.ca">CRHS web site <i class="fas fa-external-link-alt"></i></a><br>
				<a class="footlink" target="_blank" href="http://www.crhsrobotics.com/2019">CRHS Robotics web site (2019) <i class="fas fa-external-link-alt"></i></a>
			</div>
			<div class="col-lg-3 col-md-6 col-sm-12 col-12">
				<h6>IDK</h6>
				<hr class="foot-div">
				<a class="footlink" target="_blank" href="privacy_policy">Privacy Policy <i class="fas fa-external-link-alt"></i></a><br>
				<a class="footlink" target="_blank" href="how_to_make_a_site">I wanna make a site! <i class="fas fa-external-link-alt"></i></a>
			</div>
		</div>
		<p class="text-center"><a class="light-link" href="../humans.txt" target="_blank">humans.txt <i class="fas fa-external-link-alt"></i></a></p>
	</footer>
	<button onclick="backToTop()" id="backtop"><i class="fas fa-caret-up"></i></button>
	</div>
	</body>
</html>